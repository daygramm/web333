...
contract('TruffleTutorial', ([contractOwner, secondAddress, thirdAddress]) => {
        ...
        describe("message", () => {
            ...
            it("address that is not the owner fails to set a message", async () => {
                await truffleTutorial.setMessage("Hi there!", {from: secondAddress}).should.be.rejected
                await truffleTutorial.setMessage("Hi there!", {from: thirdAddress}).should.be.rejected
            })
        })
    }
)